 ...
 * Include      : 
 * Declaration  : int32_t sd_init(int32_t sd_port, uint32_t base, void *workarea, int32_t cd_port);
...
int32_t sd_init(int32_t sd_port, uint32_t base, void *workarea, int32_t cd_port)
{
    ...
    if((base != SD_SCC_IP0_BASE_ADDR) && (base != SD_SCC_IP1_BASE_ADDR)){
        return SD_ERR;
    }
    ...
    /* ==== initialize SDHI ==== */
    SD_OUTP(hndl,SD_INFO1_MASK,SD_INFO1_MASK_ALL);
    SD_OUTP(hndl,SD_INFO2_MASK,SD_INFO2_MASK_ALLP);
    info1 = SD_INP(hndl,SD_INFO1);
    SD_OUTP(hndl,SD_INFO1,(uint64_t)(info1&~SD_INFO1_MASK_TRNS_RESP));
    SD_OUTP(hndl,SD_INFO2,0x0000);
    SD_OUTP(hndl,SOFT_RST,SOFT_RST_SDRST_RESET);
    SD_OUTP(hndl,SOFT_RST,SOFT_RST_SDRST_RELEASED);

    SD_OUTP(hndl,DM_CM_INFO1_MASK,DM_CM_INFO1_MASK_ALLP);
    SD_OUTP(hndl,DM_CM_INFO2_MASK,DM_CM_INFO2_MASK_ALLP);
    SD_OUTP(hndl,DM_CM_INFO1,(uint64_t)0);
    SD_OUTP(hndl,DM_CM_INFO2,(uint64_t)0);
    ...
    SD_OUTP(hndl,HOST_MODE,HOST_MODE_64BIT_ACCESS);
    
    SD_OUTP(hndl,SD_OPTION,SD_OPTION_INIT);
...
/*****************************************************************************
 * ID           :
 * Summary      : initialize SD Driver error
 * Include      : 
 * Declaration  : static int32_t _sd_init_error(int32_t sd_port, int32_t ret)
 * Functions    : 
 *              : 
 * Argument     : int32_t sd_port : channel no (0 or 1)
 ...
 * Include      : 
 * Declaration  : int32_t sd_finalize(int32_t sd_port);
 ...
 * Include      : 
 * Declaration  : int32_t _sd_init_hndl(SDHNDL *hndl, uint32_t mode,
 *              :   uint32_t voltage)
...
int32_t _sd_init_hndl(SDHNDL *hndl, uint32_t mode, uint32_t voltage)
{
    ...
    hndl->speed_mode = 0;
    hndl->int_mode = (uint8_t)(mode & 0x1u);
    hndl->trans_mode = (uint8_t)(mode & SD_MODE_DMA);
    hndl->sup_card = (uint8_t)(mode & 0x30u);
    hndl->sup_speed = (uint16_t)(mode & 0xF040u);
