...
volatile struct st_ssif* const g_ssireg[SSIF_NUM_CHANS] = {
    ...
    else
    {
        ...
        else
        {
            ...
            if (ESUCCESS == ercd)
            {
                ...
                CPG.STBCR7.BYTE &= (uint8_t)~((uint8_t)gb_cpg_stbcr_bit[ssif_ch]);
                /* <-IPA R2.4.2 */
            ...
            if (ESUCCESS == ercd)
            {
                g_ssireg[ssif_ch]->SSIFCR.BIT.AUCKE = 1;
...
int_t SSIF_DisableChannel(ssif_info_ch_t* const p_info_ch)
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* Reset FIFO */
            g_ssireg[ssif_ch]->SSIFCR.LONG |= (SSIF_FCR_BIT_TFRST | SSIF_FCR_BIT_RFRST);
            dummy_read = g_ssireg[ssif_ch]->SSIFCR.LONG;
            ...
            /* clear status reg */
            g_ssireg[ssif_ch]->SSISR.LONG = 0u; /* ALL CLEAR */

            g_ssireg[ssif_ch]->SSIFCR.BIT.AUCKE = 0;

            /* disable ssif clock */
            core_util_critical_section_enter();

            /* ->IPA R2.4.2 : This is implicit type conversion that doesn't have bad effect on writing to 8bit register. */
            CPG.STBCR7.BYTE |= (uint8_t)gb_cpg_stbcr_bit[ssif_ch];
            /* <-IPA R2.4.2 */
            dummy_buf = CPG.STBCR7.BYTE;
            (void)dummy_buf;
            core_util_critical_section_exit();
...
void SSIF_ErrorRecovery(ssif_info_ch_t* const p_info_ch)
{
    ...
    else
    {
        ...
        else
        {
            ...
            g_ssireg[ssif_ch]->SSIFCR.LONG &= ~((uint32_t)SSIF_FCR_BIT_TIE | SSIF_FCR_BIT_RIE);
...
int_t SSIF_IOCTL_GetStatus(const ssif_info_ch_t* const p_info_ch, uint32_t* const p_status)
{
    ...
    {
        *p_status = g_ssireg[p_info_ch->channel]->SSISR.LONG;
    }
                                      ...
                                      const ssif_channel_cfg_t* const p_ch_cfg)
{
    ...
    else
    {
        ...
        if (ESUCCESS == ercd)
        {
            ...
            {
                /* slave mode */
                p_info_ch->master_enable = SSIF_CFG_SLAVE_MODE;
            }
            else
            {
                /* master mode */
                p_info_ch->master_enable = SSIF_CFG_MASTER_MODE;
            ...
            if (p_ch_cfg->clk_select == SSIF_CFG_CKS_AUDIO_CLK) {
                p_info_ch->clk_select     = 0;
            } else {
                p_info_ch->clk_select     = 1;
            }
...
static int_t SSIF_SetCtrlParams(const ssif_info_ch_t* const p_info_ch)
{
    ...
    else
    {
        ...
        /* ALL CLEAR */
        g_ssireg[ssif_ch]->SSICR.LONG = 0u;
        g_ssireg[ssif_ch]->SSISR.LONG = 0u;

        g_ssireg[ssif_ch]->SSICR.LONG = (uint32_t)(
             ...
             ((uint32_t)(p_info_ch->master_enable)      << SSIF_CR_SHIFT_MST) |
             ((uint32_t)(p_info_ch->clk_div)            << SSIF_CR_SHIFT_CKDV)
             );

        if (p_info_ch->tdm_mode == 0) {
            g_ssireg[ssif_ch]->SSIOFR.BIT.OMOD = 0x0;
        } else {
            g_ssireg[ssif_ch]->SSIOFR.BIT.OMOD = 0x1;
        }
...
static void SSIF_Reset(const uint32_t ssif_ch)
{
    g_ssireg[ssif_ch]->SSIFCR.BIT.SSIRST = 1;
    while (g_ssireg[ssif_ch]->SSIFCR.BIT.SSIRST != 1) {
        ;
    }
    g_ssireg[ssif_ch]->SSIFCR.BIT.SSIRST = 0;
    while (g_ssireg[ssif_ch]->SSIFCR.BIT.SSIRST != 0) {
        ;
    }
